-- 테이블삭제 (참조관계에서 외래키를 가진 테이블(자식테이블)을 기본키를 가진 테이블(부모테이블)보다 먼저 삭제해야 한다.)
DROP TABLE 제품;
DROP TABLE 주문;
DROP TABLE 배송;
DROP TABLE 게시판;
...
(
  ...
  내용 VARCHAR2(1000),
  작성자 VARCHAR2(20) NOT NULL REFERENCES 회원 (회원아이디),
  작성일자 DATE
);
...
(
  ...
  배송업체 CHAR(12) REFERENCES 택배업체 (택배업체사업자번호),
  배송금액 NUMBER,
  배송일자 DATE
);
...
(
  ...
  주문회원 VARCHAR2(20) REFERENCES 회원 (회원아이디),
  주문일자 DATE,
...
(
  ...
  제조사 CHAR(12) REFERENCES 제조사 (제조사사업자번호),
  주문번호 NUMBER REFERENCES 주문 (주문번호),
  창고코드 CHAR(10) REFERENCES 창고 (창고코드),
  제품수량 NUMBER,
  제품단가 NUMBER,
  카테고리 CHAR(10),
  적용할인율 NUMBER(2, 2)
